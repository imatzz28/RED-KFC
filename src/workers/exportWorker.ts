...
self.onmessage = (e) => {
    ...
    try {
        ...
        // Crear índice SIEMPRE, ya que se necesita para D y F incluso sin includeDetails
        ...
        const exportData = initialEmployees
            ...
            .map((emp: any) => {
                ...
                exportConfig.groups.forEach((gid: string) => {
                    ...
                    // D y F NO heredan del summary: si no hay nota directa en el mes, el % es 0
                    let groupAvg = 0;
                    if (gid === 'D' || gid === 'F') {
                        // Calcular desde el índice en lugar del array gigante (O(1) vs O(N))
                        const empGrades = gradeIndex.get(empId) || [];
                        const directGrades = empGrades.filter((g: any) =>
                            g.group === gid &&
                            g.month && g.month.substring(0, 7) === selectedMonth
                        );
                        if (directGrades.length > 0) {
                            const sum = directGrades.reduce((acc: number, g: any) => acc + (g.score || 0), 0);
                            groupAvg = Math.round(sum / gConfig.categories.length);
                        }
                    } else {
                        groupAvg = emp.summary ? Math.round(emp.summary[`avg_${gid.toLowerCase()}`] || 0) : 0;
                    }
