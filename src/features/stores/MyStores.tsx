...
const MyStores: React.FC = () => {
  ...
  const storesWithStats = useMemo(() => {
    return assigned.map(store => {
      ...
      const groupData: Record<string, { scores: number[], passed: number, hasGrades: boolean }> = {};
      ...
      storeEmps.forEach(emp => {
        ...
        let effective = dataService.getEffectiveGrades(emp.id, selectedMonth, store.id);
        if (effective.length === 0) {
          effective = dataService.getEffectiveGrades(emp.id, selectedMonth);
        }
        ...
        Object.entries(EVALUATION_GROUPS).forEach(([gid, gconfig]) => {
          // REGLA: El indicador All-Star (Grupo C) solo cuenta para personal con +2 meses
          ...
          if (groupGrades.length > 0) {
            ...
            groupData[gid].hasGrades = true;
          } else if (empSummary && gid !== 'D' && gid !== 'F') {
            // D y F NO heredan del summary: si no tienen nota directa en el mes, son 0
      ...
      const groupStats: Record<string, { avg: number, approvalRate: number, hasGrades: boolean }> = {};
      let totalAvgSum = 0;
      const eligibleForAllStar = storeEmps.filter(e => getSeniorityMonths(e.join_date, selectedMonth) > 2).length;
      Object.keys(EVALUATION_GROUPS).forEach(gid => {
        ...
        groupStats[gid] = { avg: roundedAvg, approvalRate: Math.round(rate), hasGrades: groupData[gid].hasGrades };
  ...
  const getStoreStatsForMonth = (storeId: string, month: string) => {
    ...
    storeEmps.forEach(emp => {
      ...
      let effective = dataService.getEffectiveGrades(emp.id, month, storeId);
      if (effective.length === 0) {
        effective = dataService.getEffectiveGrades(emp.id, month);
      }
      ...
      Object.entries(EVALUATION_GROUPS).forEach(([gid, gconfig]) => {
        ...
        if (groupGrades.length > 0) {
          ...
          // D y F NO heredan del summary: nota mensual directa o nada
          const raw = empSummary[`avg_${gid.toLowerCase()}`];
          if (raw != null && Number(raw) > 0) {
            gAvg = Number(raw);
            groupData[gid].hasGrades = true;
    ...
    const groupStats: Record<string, { avg: number, approvalRate: number, hasGrades: boolean }> = {};
    let totalAvgSum = 0;
    const eligibleForAllStar = storeEmps.filter(e => getSeniorityMonths(e.join_date, month) > 2).length;
    Object.keys(EVALUATION_GROUPS).forEach(gid => {
      const avg = groupData[gid].scores.length > 0 ? groupData[gid].scores.reduce((a, b) => a + b, 0) / groupData[gid].scores.length : 0;
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      // 1. Si el periodo solicitado difiere del mes activo en pantalla, cargar notas del mes solicitado
      if (pdfMonth !== selectedMonth) {
        await Promise.all([
          dataService.loadGradesForStore(selectedStore.id, pdfMonth),
          dataService.loadGradesSummary(pdfMonth)
        ]);
      }

      const currentSummaryMap = new Map((dataService.getGradesSummary() || []).map(s => [String(s.employee_id).trim(), s]));
      const stats = getStoreStatsForMonth(selectedStore.id, pdfMonth);
      await generateStorePdf(selectedStore, pdfMonth, employees, stats, currentSummaryMap);
      setShowPdfModal(false);
    } catch (err) {
      console.error(err);
      showAlertDialog("Error al generar el reporte PDF.");
    } finally {
      // 2. Si se consultó un mes diferente, restaurar el cache de notas del mes activo en pantalla
      if (pdfMonth !== selectedMonth && selectedStore) {
        await Promise.all([
          dataService.loadGradesForStore(selectedStore.id, selectedMonth),
          dataService.loadGradesSummary(selectedMonth)
        ]);
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.entries(EVALUATION_GROUPS).map(([id, group]) => {
            const groupStat = stats.groupStats[id];
            const groupApprovalRate = groupStat?.approvalRate ?? 0;
            const hasGrades = groupStat?.hasGrades ?? false;
            return (
              <div key={id} className={`bg-white p-6 rounded-[32px] border-2 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 group ${hasGrades && groupApprovalRate >= 90 ? 'border-emerald-50' : 'border-slate-50'}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    {hasGrades ? (
                      <p className={`text-2xl font-black italic tracking-tighter leading-none ${groupApprovalRate >= 90 ? 'text-emerald-700' : 'text-slate-800'}`}>{groupApprovalRate}%</p>
                    ) : (
                      <p className="text-lg font-black italic tracking-tighter leading-none text-slate-300">S/N</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-1.5 items-end">
                  <div className="w-full md:w-20 h-1.5 bg-slate-100 rounded-full overflow-hidden shrink-0 relative shadow-inner">
                    {hasGrades && (
                      <div className={`absolute top-0 left-0 bottom-0 transition-all duration-1000 ${groupApprovalRate >= 90 ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-red-500'}`} style={{ width: `${groupApprovalRate}%` }} />
                    )}
                  </div>
