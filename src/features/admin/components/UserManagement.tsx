...
export const UserManagement: React.FC<Props> = ({ currentUser, users, setUsers, hierarchy, restaurants, setImportStatus }) => {
  ...
  // Resolver regiones efectivas del usuario (directas o deducidas de sus zonas/tiendas asignadas)
  const getUserEffectiveRegions = (u: Partial<User>): string[] => {
    const regions = new Set<string>(u.assignedRegions || []);
    if (regions.size > 0) return Array.from(regions);

    // Deducir desde assignedZones
    (u.assignedZones || []).forEach(zoneName => {
      if (!zoneName) return;
      const targetZone = zoneName.trim().toUpperCase();
      const reg = hierarchy.regions.find(r =>
        r.zones.some(z => (z.name || '').trim().toUpperCase() === targetZone)
      );
      if (reg) {
        regions.add(reg.name);
      } else {
        const restReg = restaurants.find(r => (r.zone || '').trim().toUpperCase() === targetZone)?.region;
        if (restReg) regions.add(restReg);
      }
    });

    // Deducir desde assignedRestaurants
    (u.assignedRestaurants || []).forEach(rid => {
      const rest = restaurants.find(r => r.id === rid);
      if (rest?.region) regions.add(rest.region);
    });
  ...
  const getCoordinatorForUser = (user: User) => {
    ...
    const userRegions = getUserEffectiveRegions(user);
    const coord = users.find(u => 
      u.role === UserRole.COORDINATOR && 
      (u.assignedRegions || []).some(reg => userRegions.includes(reg))
    );
    return coord || null;
  };
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      let computedRegions = userToProcess.assignedRegions || [];
      if (userToProcess.role === UserRole.SPECIALIST && computedRegions.length === 0) {
        computedRegions = getUserEffectiveRegions(userToProcess as User);
      }
      ...
      const userToSave: User = selectedUser
        ? { 
            ...
            assignedRegions: computedRegions,
  ...
  const assignedCoordinatorsCount = useMemo(() => {
    ...
    return coordinators.filter(c => 
      visibleUsers.some(s => s.role === UserRole.SPECIALIST && getUserEffectiveRegions(s).some(reg => (c.assignedRegions || []).includes(reg)))
    ).length;
  }, [visibleUsers, hierarchy, restaurants]);
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-slate-50/30 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        {(() => {
                          const effectiveRegions = getUserEffectiveRegions(u);
                          return (
                            <div className="flex flex-col">
                              <div className="flex items-center gap-1 text-xs text-slate-600 font-bold">
                                <MapPin className="w-3.5 h-3.5 text-blue-500 shrink-0" />
                                <span className="truncate max-w-[200px]" title={effectiveRegions.join(', ') || (u.assignedZones || []).join(', ')}>
                                  {effectiveRegions.join(', ') || (u.assignedZones || []).join(', ') || '—'}
                                </span>
                              </div>
                              {u.assignedZones && u.assignedZones.length > 0 && effectiveRegions.length > 0 && (
                                <span className="text-[9px] text-slate-400 font-medium pl-4.5 truncate max-w-[200px]" title={u.assignedZones.join(', ')}>
                                  Zona: {u.assignedZones.join(', ')}
                                </span>
                              )}
                            </div>
                          );
